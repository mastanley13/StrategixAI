...
export function setupHeroAnimation(canvas: HTMLCanvasElement): () => void {
  ...
  // Add floating geometric shapes for a tech feel
  interface ShapeItem {
    mesh: THREE.Mesh;
    rotationSpeed: {
      x: number;
  ...
  for (let i = 0; i < shapes.length; i++) {
    for (let j = i + 1; j < shapes.length; j++) {
      // Only connect some shapes
      if (Math.random() > 0.85) {
        const pointsGeometry = new THREE.BufferGeometry();
        const positions = new Float32Array([
          shapes[i].mesh.position.x,
          shapes[i].mesh.position.y,
          shapes[i].mesh.position.z,
          shapes[j].mesh.position.x,
          shapes[j].mesh.position.y,
          shapes[j].mesh.position.z
        ]);
  ...
  function animate() {
    ...
    shapes.forEach(shape => {
      ...
      // Float movement
      shape.mesh.position.y += Math.sin(time + shape.movementOffset) * shape.movementSpeed;
    });
    
    // Update connection lines
    connections.forEach(connection => {
      const startShape = shapes[connection.start];
      const endShape = shapes[connection.end];
      
      const positions = connection.line.geometry.attributes.position.array as Float32Array;
      
      positions[0] = startShape.mesh.position.x;
      positions[1] = startShape.mesh.position.y;
      positions[2] = startShape.mesh.position.z;
      
      positions[3] = endShape.mesh.position.x;
      positions[4] = endShape.mesh.position.y;
      positions[5] = endShape.mesh.position.z;
